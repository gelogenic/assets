$(function () {
    ...
    $.ajax({
        type: 'GET',
        url: '/assets/xml/usaMapSettings.xml',
        ...
        for (var i = 0; i<pinNames.length; i++) {
            ...
            pin.mouseover(function (e) {
                ...
                var id = $(this.node).attr('id');
                                                            ...
                                                            displayText = pinNames[id];
                                                            $('.point').html(displayText).css({
                                                                           left: mouseX - 50,
                                                                           top: mouseY - 70
                                                            }).fadeIn();
            });
        ...
        for (var state in usamappaths) {
            ...
            hitArea.mouseover(function (e) {
                ...
                if (stateModes[id] != 'OFF') {
                    ...
                    // When the mouse hovers over a state, display the state name and the CHIP participation rate.
                                                                           displayText = stateNames[id];
                                                                           if (behavior === "displayRate") {
                                                                                          chipParticipation.setStateData(displayText);
                                                                                          if (chipParticipation.getRate() !== null) {
                                                                                                         displayText += ("<br/>" + chipParticipation.getRate());
                                                                                          }
                                                                           }
            ...
            hitArea.click(function (e) {
                                                            if (behavior === "gotoChip") {
                                                                           e.stopPropagation();
                                                                           var id = $(this.node).attr('id');
                                                                           //displayText = stateNames[id] ;
                                                                           //console.log("click on " + displayText);
                                                                           window.open(stateURLs[id], '_self');
                                                            }
            });
...
// used to filter CHIP Participation
function filterByState(record) {
  if ('state' in record && typeof(record.state) === 'string' && isNaN(record.state) && record.state.toUpperCase() === chipParticipation.stateName.toUpperCase()) {
    return true;
  } else {
    return false;
  }
...
var chipParticipation = {
    allStatesChipParticipation : readDataFile("/assets/data/chip-participation-by-state.js"),
    //get state record from allStatesChipParticipation array
    setStateData : function (state) {
                              this.stateName = state;
                              var list = this.allStatesChipParticipation.filter(filterByState);
                              if (list.length === 1) {
                                             this.stateData = list[0];
                              } else {
                                             this.stateData = null;
                              }
    },
    ...
    //display Chip Participation data 
    getRate : function () {
                              ret = null;
        if (this.getStateData()) {
                                             ret = this.getStateData().participation;
        }
                              return ret;
    }
};

// One map supports two different behaviors
function getBehavior(record) {
               if (document.location.pathname  === "/about/participation-rates/index.html") {
                              return "displayRate"; // display the CHIP participation rate for the state on hover
               } 
               if (document.location.pathname  === "/state/index.html") {
                              return "gotoChip"; // goto the state-specific CHIP web page
               } else {
                   return ""; 
               }
}
